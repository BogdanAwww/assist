...
export function queryCostPlugin(opts: {schema: GraphQLSchema; maxComplexity: number | string}) {
	return {
		async requestDidStart(/* {context} */): Promise<any> {
			let complexity = 0;

			// also you may get user from `context` and
			// provide custom `maxComplexity` for him.
			const maxComplexity =
				(typeof opts.maxComplexity === 'string' ? parseInt(opts.maxComplexity) : opts.maxComplexity) || 100000;
			return {
